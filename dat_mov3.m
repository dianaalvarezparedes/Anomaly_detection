...
vec_index_values=unique(b.bts_rnc_adj,'rows');
for id=vec_index_values                                               %identificadores buscados, reemplazar por los reales, listado
...
tab_nom = readtable('clean_rbs.txt'); 
...
for i_mea_los=1:size(mat_mea_los,2)
   ...
   xlabel('$Lag$ $30$ $min.$','interpreter','latex')
   ylabel('$Packet$ $loss$','interpreter','latex')
   leg_fil=fun_bus_nom_v3(num_leg(i_mea_los),tab_nom);
   tex_nombre=[leg_fil;tex_nombre];
   legend(leg_fil)
   saveas(fig,['C:\Users\juan\Dropbox\Proyecto TFM\' 'graficosTFM\intento4\' leg_fil '4' ' .png']);
   close all
end
...
%%%
    ...
    saveas(fig2,['C:\Users\juan\Dropbox\Proyecto TFM\' 'graficosTFM\intento4\' 'corr3_' num2str(ind_cor)  '.png']);

    close all

    end
end
writetable(tabla_rbs, '.\graficosTFM\intento4\tab_res_nom_per.csv','QuoteStrings',true);